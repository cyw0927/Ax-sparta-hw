...
def part1_list_and_loop():
    ...
    # for문으로 리스트의 점수를 하나씩 꺼냅니다.
    for score in scores:
        # 점수가 60점 이상인지 조건을 확인합니다.
        if score >= 60:
            result = "합격"
...
"""
    ...
    # split()은 공백과 줄바꿈을 기준으로 문자열을 단어 단위로 나눕니다.
    words = sentence.split()

    # 단어별 등장 횟수를 저장할 빈 딕셔너리입니다.
    word_counts = {}

    # 단어를 하나씩 확인합니다.
    for word in words:
        # 이미 딕셔너리에 있는 단어라면 기존 횟수에 1을 더합니다.
        if word in word_counts:
            word_counts[word] = word_counts[word] + 1
        # 처음 등장한 단어라면 1을 저장합니다.
        else:
            word_counts[word] = 1
...
def count_words(sentence):
    """
    문자열을 전달받아 단어별 등장 횟수를 딕셔너리로 반환합니다.
    """
    words = sentence.split()
    word_counts = {}
...
def print_words_over_count(word_counts, min_count):
    """
    word_counts에서 min_count 이상 등장한 단어만 출력합니다.
    """
    found = False
...
def search_word_with_exception(word_counts, search_word):
    """
    존재하지 않는 Key를 []로 조회하면 KeyError가 발생할 수 있습니다.
    try-except로 오류를 처리해 프로그램이 중단되지 않게 합니다.
    """
    try:
...
def main():
    ...
    # Part 2와 Part 3에서 같은 문장을 분석했으므로 결과가 같은지 검증합니다.
    print_title("최종 검증")
